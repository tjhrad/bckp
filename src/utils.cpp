...
int unidirectional_directory_backup(std::string main_directory, std::string backup_directory)
{
    ...
    for (const auto & entry : std::filesystem::recursive_directory_iterator(main_directory))
    {
        if ( std::filesystem::is_directory(entry.path()) )
        {
            ...
            std::filesystem::path temporary_stem = dir.erase(dir.find(main_directory),main_directory.length());//entry.path().string()//entry.path().stem();
    ...
    //TODO: Check if backup directory has files or folders that don't exist in main directory
